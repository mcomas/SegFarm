...
group_by(res) %>%  summarize(
  ...
  'Urgents' = sprintf("    %d (%5.2f%% de les dispensacions)", sum(dispensacio.urgent == 'Si'), 100*mean(dispensacio.urgent == 'Si')))
```
...
```{r, echo=FALSE, comment="    "}
data.frame(
  group_by(res, grup.terapeutic) %>%  summarize(
  'Dispensacions' = sprintf("    %d (%5.2f%% del total)", length(idenquesta), 100 * length(idenquesta) / nrow(res)), 
  'Urgents' = sprintf("    %d (%5.2f%% de les dispensacions)", sum(dispensacio.urgent == 'Si'),  100*mean(dispensacio.urgent == 'Si'))))
```

```{r, echo=FALSE, comment="    "}
mutate(res, 'atc' = substr(grup.terapeutic, 1, 3)) %>% group_by(atc) %>%  summarize(
  'Dispensacions' = sprintf("    %d (%5.2f%% del total)", length(idenquesta), 100 * length(idenquesta) / nrow(res)), 
  'Urgents' = sprintf("    %d (%5.2f%% de les dispensacions)", sum(dispensacio.urgent == 'Si'),  100*mean(dispensacio.urgent == 'Si')))
```

```{r, echo=FALSE, comment="    "}
mutate(res, 'atc' = substr(grup.terapeutic, 1, 1)) %>% group_by(atc) %>%  summarize(
  'Dispensacions' = sprintf("    %d (%5.2f%% del total)", length(idenquesta), 100 * length(idenquesta) / nrow(res)), 
  'Urgents' = sprintf("    %d (%5.2f%% de les dispensacions)", sum(dispensacio.urgent == 'Si'),  100*mean(dispensacio.urgent == 'Si')))
```
...
```{r, echo=FALSE, comment="    "}
data.frame(
  group_by(res, tipus.producte) %>%  summarize(
  'Dispensacions' = sprintf("    %d (%5.2f%% del total)", length(idenquesta), 100 * length(idenquesta) / nrow(res)), 
  'Urgents' = sprintf("    %d (%5.2f%% de les dispensacions)", sum(dispensacio.urgent == 'Si'),  100*mean(dispensacio.urgent == 'Si'))))
```
...
group_by(res, idenquesta) %>% summarize(
  ...
  'dispensacio.urgent' = sum(dispensacio.urgent == 'Si') > 0) %>% summarize(
    ...
    'Amb disp. urgent' = sprintf("    %d (%5.2f%% del total)", sum(dispensacio.urgent), length(idenquesta), 100*mean(dispensacio.urgent)),
